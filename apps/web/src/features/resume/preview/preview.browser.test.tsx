...
describe("ResumePreviewClient", () => {
	...
	it("logs PDF generation errors so preview failures are diagnosable", async () => {
		const error = new Error("PDF failed");
		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});

		previewMock.toBlob.mockRejectedValue(error);
		...
		await waitFor(() => {
			expect(errorSpy).toHaveBeenCalledWith("Failed to generate resume PDF preview", error);
		});

		expect(screen.getByRole("img", { name: "Loading resume page 1 of 4" })).toBeTruthy();
