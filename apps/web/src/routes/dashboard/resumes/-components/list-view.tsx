...
export function ListView({ onLocalChange, resumes }: ListViewProps) {
		...
		<div className="flex flex-col gap-y-1">
			...
			<AnimatePresence initial={false} mode="popLayout">
				{resumes?.map((resume, index) => (
					...
					>
						<ResumeListItem resume={resume} onLocalChange={onLocalChange} />
					</m.div>
				))}
			</AnimatePresence>
		</div>
...
function ResumeListItem({ onLocalChange, resume }: ResumeListItemProps) {
	...
	return (
		<div className="flex items-center gap-x-2">
			...
			<ResumeDropdownMenu resume={resume} align="end" onLocalChange={onLocalChange}>
