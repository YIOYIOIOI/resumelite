...
describe("useDialogStore", () => {
	...
	describe("openDialog", () => {
		it("uses schema entries from domain registries", () => {
			expect(dialogSchemaRegistries.map((registry) => registry.domain)).toEqual(["resume", "experience"]);
	...
	describe("closeDialog", () => {
		it("immediately sets open to false", () => {
			useDialogStore.setState({
				open: true,
				activeDialog: { type: "resume.import", data: undefined },
			});
	...
	describe("onOpenChange", () => {
		...
		it("closes immediately when no onBeforeClose handler", () => {
			useDialogStore.setState({ open: true, activeDialog: { type: "resume.import", data: undefined } });
